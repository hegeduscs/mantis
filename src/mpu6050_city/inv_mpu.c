		...
		unsigned char reg_addr, unsigned short len, const unsigned char *data_ptr)
{
	...
	{
		if(TM_I2C_WriteMulti(I2C_SENS, slave_addr << 1, reg_addr, data_ptr, len) == TM_I2C_Result_Ok)
		{
			return 0;
		}
		//delay_ms(100);
		...
		unsigned char reg_addr, unsigned short len, const unsigned char *data_ptr)
{
	...
	{
		if(TM_I2C_ReadMulti(I2C_SENS, slave_addr << 1, reg_addr, data_ptr, len) == TM_I2C_Result_Ok)
		{
			return 0;
		}
		//delay_ms(100);
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    TM_I2C_Init(I2C_SENS, GPIO_PIN2, I2C_CLOCK);

    if (TM_I2C_IsDeviceConnected(I2C_SENS, MPU6050_I_AM << 1) != TM_I2C_Result_Ok)
    {
    		/* Return error */
    		return -1;
    }
